...
import s from './PasswordWindow.module.scss';
export const PasswordWindow = () => {
  ...
  return (
    <div className={s.passwordWindow_Wrapper}>
      <span className={s.password_text}>Тут буде пароль</span>
      <button
        className={animation ? `${s.reboot_icon_button} ${s.animation} ` : s.reboot_icon_button}
      ...
      <button className={s.copy_button}>КОПИРОВАТЬ</button>
    </div>
  );
};
